...
def test_space_separated_init(rows_num, columns_num, attribute, result):
    assert input_handler._IdentityMatrix(rows_num, columns_num).__dict__[attribute] == result
...
def test_initialization_broken(rows_num, columns_num, attribute, expected_exception):
    with pytest.raises(expected_exception):
        input_handler._IdentityMatrix(rows_num, columns_num).__dict__[attribute]
